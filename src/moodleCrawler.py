...
def checkBool(variable, name):
    if variable == "true" or variable == "false":
        ...
        log(
            "Error parsing Variable. Please check the config file for variable: " + name + ". This variable should be 'true' or 'false'",
            0)
        exit()
...
def checkInt(variable, name):
    if variable.isdigit():
        ...
        log(
            "Error parsing Variable. Please check the config file for variable: " + name + ". This variable should be an integer",
            0)
        exit()
...
def checkConf(cat, name):
    ...
    try:
        ...
        log(
            "Variable in config is missing. Please set the variable: " + name + " in the config file in section: " + cat,
            0)
        exit()
...
if not os.path.isdir(root_directory):
    log(
        "Error parsing Variable. Please check the config file for variable: root_dir. This variable should be a path to an existing folder.",
        0)
    exit()
...
def addFileToLog(pageLink, filePath):
    ...
    logFileWriter.write(
        datetime.now().strftime('%d.%m.%Y %H:%M:%S') + " " + pageLink + " saved to '" + filePath + "'\n")
    logFileWriter.close()
    global logFile
...
def addHashToLog(pageDir, calcHash):
    logFileWriter = io.open(crawlHistoryFile, 'a')
    logFileWriter.write(
        datetime.now().strftime('%d.%m.%Y %H:%M:%S') + " " + pageDir + " calculated hash " + calcHash + "\n")
    logFileWriter.close()
    global logFile
    logFileReader = io.open(crawlHistoryFile, 'r')
    logFile = logFileReader.read()
...
def findOwnCourses(myCoursesURL):
    ...
    for course_string in courseNameList:
        ...
        if not onlyCrawlCoursesCheck(course_link):
            log(
                "Course " + course_name + " will not be crawled because the course id is not given in option 'onlycrawlcourses'.",
                3)
            continue

        if dontCrawlCoursesCheck(course_link):
            log(
                "Course" + course_name + " will not be crawled because the course id is given in option 'dontcrawlcourses'.",
                3)
            continue
...
def searchfordumpsSpecific(filepath, fileName, filetype, pathtoSearch):
    ...
    filesBySizeSpe = {}
    log(
        'Scanning directory "' + pathtoSearch + '" (file: "file://' + filepath + '", filename: "' + fileName + '", filetype: "' + filetype + '")....',
        5)
...
def logDuplicates(dubPath, oriPath):
    ...
    if os.path.isfile(dubLogPath):
        ...
        if not dubPath in dubLog:
            ...
            log(
                'This duplicates were found before and loged in the "file://' + dubLogPath + '" file earlier. Inform the project maintainer pleace.',
                5)
        ...
        dubLogWriter = io.open(dubLogPath, 'ab')
        dubLogWriter.write(
            datetime.now().strftime('%d.%m.%Y %H:%M:%S') + " " + dubPath + " was found in this place " + oriPath + "\n")
        dubLogWriter.close()
...
Name[en_US]=""" + extname)
    ...
    if boolExternalLinkStored == True:
        logFileWriter = io.open(crawlHistoryFile, 'a')
        logFileWriter.write(
            datetime.now().strftime('%d.%m.%Y %H:%M:%S') + " External: " + extlink + " saved to '" + file_name + "'\n")
        logFileWriter.close()
        global logFile
        logFileReader = io.open(crawlHistoryFile, 'r')
        logFile = logFileReader.read()
...
def crawlMoodlePage(pagelink, pagename, parentDir, calledFrom, depth=0, forbidrecrusionfor=[]):
    ...
    log("Check page: '" + pagelink + "' named: '" + pagename + "' found on: '" + calledFrom + "' depth: " + str(
    ...
    if usehistory == "true" and pagelink in logFile:
        log("This link was crawled in the past. I will not recrawl it, change the settings if you want to recrawl it.",
            3)
        return
    ...
    if not domainMoodle in pagelink:
        ...
        if downloadExternals == "false":
            log(
                "Ups this is an external link. I do not crawl external links. Change the settings if you want to crawl external links.",
                3)
            return
    ...
    if isexternlink == False and not domainMoodle in realurl:
        ...
        if usehistory == "true" and realurl in logFile:
            log(
                "This link was crawled in the past. I will not recrawl it, change the settings if you want to recrawl it.",
                3)
            return
    ...
    pageIsHtml = False
    if "text/html" in responsePageLink.info().get('Content-Type') or pageFileName[-4:] == ".php" or pageFileName[
                                                                                                    -5:] == ".html":
        pageIsHtml = True
    ...
    if pageIsHtml == True and reLoginOnFile == "true" and simpleMoodleCheck(PageLinkContent):
        ...
        if loginStatus == 0:  # Not logged in
            log(
                "Ups, there went something wrong with the moodle login - this is bad. If this happens again please contect the project maintainer.",
                0)
    ...
    if pageIsHtml == True and isexternlink == False:
        ...
        if not page_links_Soup is None:
            ...
            # only main page
            PageLinkContent = "<!DOCTYPE html> <html>" + str(
                moodlePageHeader) + "<body class='format-topics path-mod path-mod-assign safari dir-ltr lang-de yui-skin-sam yui3-skin-sam  pagelayout-incourse category-246 has-region-side-pre used-region-side-pre has-region-side-post empty-region-side-post side-pre-only jsenabled'>" + str(
                page_links_Soup) + "</body></html>"
    ...
    if pageIsHtml == True:
        ...
        if usehistory == "true" and (pageDir + " calculated hash " + calcHash) in logFile:
            log(
                "This page was saved in the past. I will not resave it, change the recrawl settings if you want to resave it.",
                3)
            doSave = False
        else:
...
if authentication_url.split('?')[0][-16:] == "/login/index.php":
    ...
    useSpecpath = True
    log(
        "This script will probably not work. Please use an authentication URL that ends with /login/index.php or contact the project owner.")
...
if useauthstate == "true":
    ...
    if (not len(formsLogin) == 1):
        log(
            "This type of AuthState is not yet suportet! Please conntect the Developer and send him this Debug Informations:")
